...
function atmosphere_responsive_menu_settings() {
	...
	$settings = array(
		'topMenu'          => __( 'Top menu', 'atmosphere-pro' ),
		'topMenuIconClass' => 'ionicons-before ion-ios-arrow-down',
...
add_theme_support( 'genesis-menus', array( 'top-menu' => __( 'Top Menu', 'atmosphere-pro' ), 'primary' => __( 'Header Menu', 'atmosphere-pro' ), 'secondary' => __( 'Footer Menu', 'atmosphere-pro' ) ) );
...
// Add Top Navigation
add_action( 'genesis_header_right', 'add_top_nav_genesis' ); 
function add_top_nav_genesis() {
	...
	if ( has_nav_menu( 'top-menu' ) ) {
			...
			$args = array(
				...
				'container' => '',
				'menu_class' => genesis_get_option('nav_superfish') ? 'top-menu genesis-nav-menu js-superfish' : 'top-menu menu genesis-nav-menu',
				'echo' => 0
			);

			$nav = wp_nav_menu( $args );

			$nav_output = sprintf( '<nav class="top-nav-menu">%1$s</nav>', $nav );

			echo apply_filters( 'do_top_nav', $nav_output, $nav, $args );
...
genesis_register_sidebar( array(
	...
	'name'        => __( 'Front Page Slider', 'atmosphere-pro' ),
	'description' => __( 'This is the home slider section on the front page.', 'atmosphere-pro' ),
) );
genesis_register_sidebar( array(
	'id'          => 'front-page-1',
...
function sp_footer_creds_filter( $creds ) {
	$creds = '[footer_copyright] EHE International. All Rights Reserved.';
	return $creds;
}
