...
/**
 * Template Name: Home Photo V 2.0
 *
 */

add_action( 'genesis_meta', 'atmosphere_home_default_genesis_meta' );
/**
 * Add widget support for homepage. If no widgets active, display the default loop.
 *
 * @since 1.0.0
 */
function atmosphere_home_default_genesis_meta() {
	...
	if ( is_active_sidebar( 'front-page-1' ) || is_active_sidebar( 'front-page-2' ) || is_active_sidebar( 'front-page-3' ) || is_active_sidebar( 'front-page-4' ) ) {
		...
		add_action( 'wp_enqueue_scripts', 'atmosphere_home_default_backstretch_scripts' );
		...
		add_action( 'genesis_loop', 'atmosphere_home_default_widgets' );
...
function atmosphere_home_default_backstretch_scripts() {

	$image = get_option( 'atmosphere-front-image', sprintf( '%s/images/front-page.jpg', get_stylesheet_directory_uri() ) );
...
function atmosphere_home_default_widgets() {
